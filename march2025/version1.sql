...
CREATE TABLE academy_progress_report (
    ...
    INDEX idx_course_completed_at (course_completed_at)
);
...
DROP TEMPORARY TABLE IF EXISTS user_groups;
CREATE TEMPORARY TABLE user_groups AS
SELECT 
    user_id,
    CAST(SUBSTRING_INDEX(meta_key, '_', -1) AS UNSIGNED) AS group_id
FROM wp_usermeta
WHERE meta_key LIKE 'learndash_group_users_%';

-- Get group course access
DROP TEMPORARY TABLE IF EXISTS group_courses;
CREATE TEMPORARY TABLE group_courses AS
SELECT 
    post_id AS group_id,
    meta_value AS course_id
FROM wp_postmeta
WHERE meta_key = 'learndash_group_enrolled_courses';

-- Get all enrolled users, including via direct enrollment, activity, and group membership
DROP TEMPORARY TABLE IF EXISTS enrolled_users;
CREATE TEMPORARY TABLE enrolled_users AS
-- Direct course enrollments
SELECT DISTINCT 
    um.user_id, 
    CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(um.meta_key, '_', 3), '_', -1) AS UNSIGNED) AS course_id
FROM wp_usermeta um
WHERE um.meta_key LIKE 'course_%_access_from'
...
SELECT DISTINCT
    ug.user_id,
    CAST(gc.course_id AS UNSIGNED) AS course_id
FROM user_groups ug
JOIN group_courses gc ON ug.group_id = gc.group_id

UNION

-- Activity-based enrollment detection
SELECT DISTINCT user_id, course_id
FROM wp_learndash_user_activity
WHERE activity_type IN ('course', 'lesson', 'topic', 'quiz');

-- Get the count of completed items per user and course and course completion timestamps
...
SELECT 
    ...
    MAX(CASE WHEN activity_type = 'course' AND activity_completed > 0 THEN activity_completed ELSE NULL END) AS course_completed_at
FROM wp_learndash_user_activity
...
SELECT 
    ...
    CASE WHEN uc.course_completed = 1 THEN FROM_UNIXTIME(uc.course_completed_at) ELSE NULL END AS course_completed_at
FROM valid_users vu
